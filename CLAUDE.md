...
- **测试：** Vitest + 场景式集成测试 + property-based invariant tests（306个测试，9个测试文件）
- **包管理：** npm
...
│   └── mock-negotiator.ts        # 确定性NegotiationParticipant（测试用）
...
│   ├── auction.ts               # 密封拍卖（first-price + Vickrey + Dutch）
│   ├── llm-auction.ts           # LLM竞价agent的拍卖场景
│   ├── multi-party.ts           # 多方联盟谈判demo
│   └── events-demo.ts           # 事件日志可观测性demo
└── cli.ts                        # 入口
...
├── auction.test.ts               # 114个测试（含Dutch拍卖）
├── invariants.test.ts            # 27个property-based不变量测试
├── event-log.test.ts             # 28个测试
└── multi-party.test.ts           # 34个测试
```
...
npx tsx src/scenarios/auction.ts --type dutch     # mock agent，Dutch降价拍卖
npm run llm-auction                               # LLM agent，first-price
npm run llm-auction -- --type vickrey             # LLM agent，Vickrey

# 多方谈判
npm run multi-party                               # 3个agent，默认配置
npx tsx src/scenarios/multi-party.ts --agents 5  # 5个agent
...
- [x] Dutch拍卖 (降价拍卖，第一个接受的bidder赢得当前价格)
- [x] 事件日志系统 (event-log.ts — subscribe/query/export，全协议层可观测)
- [x] 多方谈判协议 (multi-party.ts — 3+个agent，共识阈值，round-robin提案)
- [x] 306个测试全通过（9个测试文件）
- [ ] ZK commitment协议
